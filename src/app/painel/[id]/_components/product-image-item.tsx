...
import type { PhotoItem } from "./image-section";
...
export function ProductImageItem({
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const { message } = await axios
        .post(
          `/api/supplier-panel/${supplierPanelId}/image/${photo.spPhotoId}`,
          {
            fileKey: key,
          },
        )
  ...
  const handleReview = async (status: SupplierPanelPhotoStatus) => {
    ...
    try {
      const { message } = await axios
        .post(
          `/api/supplier-panel/${supplierPanelId}/image/${photo.spPhotoId}/review`,
          {
            status,
            rejectionReason:
  ...
  const imageContainerClass =
    "relative w-full max-w-sm aspect-square mx-auto";

  const needsUpload =
    photo.status === SupplierPanelPhotoStatus.PENDING ||
    photo.status === SupplierPanelPhotoStatus.REJECTED;

  const showExistingImage =
    !!photo.imageUrl &&
    (photo.status === SupplierPanelPhotoStatus.SUBMITTED ||
      ...
      <div className="flex flex-col items-center gap-3">
        ...
        {showExistingImage && !previewUrl && (
          <div className="w-full flex flex-col items-center space-y-4 animate-in fade-in">
            ...
            {isSellerView &&
              photo.status === SupplierPanelPhotoStatus.SUBMITTED && (
                <div className="w-full max-w-sm mt-4 animate-in fade-in slide-in-from-top-2">
                  {!showRejectForm ? (
        ...
        {photo.status === SupplierPanelPhotoStatus.REJECTED && (
          <div className="w-full max-w-sm bg-destructive/10 border border-destructive/20 text-destructive text-sm p-3 rounded-lg flex items-start gap-2 mb-2">
